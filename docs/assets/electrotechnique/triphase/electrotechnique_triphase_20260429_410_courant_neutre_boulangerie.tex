...
Nous allons utiliser une grille de repère pour dessiner ces vecteurs et de les additionner afin de connaître la valeur du courant dans le neutre (résultante) $ \vv{I_{N}} = \vv{I_{ph1}} + \vv{I_{ph2}} + \vv{I_{ph3}}$
...
Tracer l'addition vectorielle  sur la grille $\vv{I_{ph1}} + \vv{I_{ph2}} + \vv{I_{ph3}} = \vv{I_{N}}$\\
Le but est de trouver le module (longueur) de $I_N$ et son argument (angle)\\
L'équation $\vv{I_{ph1}} + \vv{I_{ph2}} + \vv{I_{ph3}} - \vv{I_{N}} = \vv{0}\\$\\
\textbf{Comprendre le signe « $-$ » dans le domaine vectoriel}\\
Le symbole « $-$ » devant un vecteur (ex: $-\vec{I}_{\text{N}}$) ne se dessine \textbf{pas} sur le graphique. Il indique une \textbf{rotation de $180^\circ$} : le vecteur garde sa longueur et sa direction, mais sa flèche pointe dans le sens exactement opposé.
...
Voilà la construction de $-\vv{I_N}$ est terminée.\\
